...
class DataProcessor:
    ...
    def generate_coefficients(self, data):
        ...
        for item in adjustable:
            ...
            if tier == "large":
                base = self.rng.uniform(0.005, 0.01)
            elif tier == "medium":
                base = self.rng.uniform(0.01, 0.02)
            else:
                base = self.rng.uniform(0.02, 0.03)
    ...
    def calibrate_total_price(self, data, target_discount_pct=None, tolerance=0.01):
        if target_discount_pct is None:
            return data

        original_total = sum(item["price"] or 0 for item in data)
        target_total = original_total * (1 - target_discount_pct / 100)

        float_limits = {"large": 0.01, "medium": 0.02, "small": 0.03}
    ...
    def basic_quality_check(self, data):
        adjustable = [item for item in data if not item["is_locked"]]
        ...
        small_floats = [
            abs((item["adjusted_price"] - item["price"]) / item["price"] * 100)
        ...
        up_items = [
            (item, (item["adjusted_price"] - item["price"]) / item["price"] * 100)
            for item in adjustable
            if item["adjusted_price"] > item["price"]
        ]
        down_items = [
            (item, (item["adjusted_price"] - item["price"]) / item["price"] * 100)
            for item in adjustable
        ...
        if up_items:
            ...
            max_up = {"code": max_up_item[0]["code"], "percentage": round(max_up_item[1], 2)}
        else:
            max_up = {"code": "", "percentage": 0}

        if down_items:
            max_down_item = min(down_items, key=lambda x: x[1])
            max_down = {"code": max_down_item[0]["code"], "percentage": round(max_down_item[1], 2)}
        else:
            max_down = {"code": "", "percentage": 0}

        float_pcts = [
            (item["adjusted_price"] - item["price"]) / item["price"] * 100
            for item in adjustable
        ]
        prices = [item["price"] for item in adjustable]

        if len(float_pcts) >= 2:
            rho, _ = spearmanr(float_pcts, prices)
            if np.isnan(rho):
                rho = 0.0
        else:
            rho = 0.0
        ...
        return {
            "float_range": {
                "large_max_float": large_max_float,
                "small_min_float": small_min_float,
                "max_up": max_up,
                "max_down": max_down,
            },
            "tier_compliance": {
                "spearman_rho": round(float(rho), 4),
                "is_negative_correlated": float(rho) < 0,
            },
        }
